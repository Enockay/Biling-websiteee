...
export default function CreditsNavbar({ onTrial }) {
    ...
    <nav className="bg-[#111111] border-b border-gray-800 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
            ...
            <li key={label}>
              <Link
                ...
                className={`relative px-4 py-5 text-sm font-medium transition-colors block ${
                  active
                    ? 'text-white after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-amber-500'
        ...
        <div className="hidden md:block">
          <button
            onClick={onTrial}
            className="px-5 py-2 text-sm font-bold text-white bg-gray-900 border border-gray-700 rounded-lg hover:bg-gray-700 transition-colors"
          >
            Get Started
          </button>
        </div>

        <button onClick={() => setOpen(!open)} className="md:hidden p-2 text-gray-400 hover:text-white rounded-md transition-colors">
          {open ? (
      ...
      {open && (
        <div className="md:hidden border-t border-gray-800 bg-[#111111] px-4 pb-4">
              ...
              <li key={label}>
                <Link
                  ...
                  className={`block px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${
                    active ? 'bg-amber-500/10 text-amber-400' : 'text-gray-400 hover:text-white hover:bg-gray-800'
                  }`}
                >
                  {label}
                </Link>
          ...
          <div className="pt-3 border-t border-gray-800 mt-3">
            <button onClick={() => { setOpen(false); onTrial?.() }} className="w-full py-2.5 bg-amber-500 text-gray-900 font-bold rounded-md text-sm">
              Get Started
            </button>
          </div>
        </div>
