...
export default function BillingNavbar({ onTrial }) {
  ...
  return (
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
            ...
            <li key={label}>
              <Link
                ...
                className={`relative px-4 py-5 text-sm font-medium transition-colors block ${
                  active
                    ? 'text-gray-900 font-bold after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-amber-500'
        ...
        <button onClick={() => setOpen(!open)} className="md:hidden p-2 text-gray-600 rounded-md hover:bg-gray-100 transition-colors">
          {open
      ...
      {open && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 pb-4">
          <ul className="flex flex-col gap-1 pt-2">
            {links.map(({ label, href, active }) => (
              <li key={label}>
                <Link to={href} onClick={() => setOpen(false)} className={`block px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${active ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50'}`}>{label}</Link>
              </li>
            ))}
          </ul>
