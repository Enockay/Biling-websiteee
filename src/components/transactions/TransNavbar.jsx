...
export default function TransNavbar({ onTrial }) {
    ...
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
            ...
            <li key={label}>
              <Link
                ...
                className={`relative px-4 py-5 text-sm font-medium transition-colors block ${
                  active
                    ? 'text-gray-900 after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-red-500'
        ...
        <div className="hidden md:flex items-center gap-3">
          <button
            onClick={onTrial}
            className="px-5 py-2 text-sm font-bold text-white bg-gray-900 rounded-lg hover:bg-gray-700 transition-colors"
          >
            Get Started
          </button>
        </div>
      ...
      {open && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 pb-4">
          ...
          <div className="pt-3 border-t border-gray-100 mt-3">
            <button onClick={() => { setOpen(false); onTrial?.() }} className="w-full py-2.5 bg-gray-900 text-white font-bold rounded-md text-sm">
              Get Started
            </button>
          </div>
        </div>
